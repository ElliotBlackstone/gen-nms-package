// This file is derived from torchvision's iou_nms_kernel.cu and has been modified.
...
__device__ inline bool devDIoU(
  ...
  // areas
  acc_T Sa = ((acc_T)a[2] - (acc_T)a[0]) * ((acc_T)a[3] - (acc_T)a[1]);
  acc_T Sb = ((acc_T)b[2] - (acc_T)b[0]) * ((acc_T)b[3] - (acc_T)b[1]);

  acc_T uni = Sa + Sb - interS;
  if (uni <= (acc_T)0) {
    return false; // treat overlap as 0 => never suppress
  }
  acc_T iou = interS / uni;

  // centers (in acc_T)
  acc_T cax = ((acc_T)a[0] + (acc_T)a[2]) * (acc_T)0.5;
  acc_T cay = ((acc_T)a[1] + (acc_T)a[3]) * (acc_T)0.5;
  acc_T cbx = ((acc_T)b[0] + (acc_T)b[2]) * (acc_T)0.5;
  acc_T cby = ((acc_T)b[1] + (acc_T)b[3]) * (acc_T)0.5;

  acc_T dx = cax - cbx;
  acc_T dy = cay - cby;
  acc_T rho2 = dx*dx + dy*dy;

  // enclosing diagonal squared + eps
  acc_T ex1 = min((acc_T)a[0], (acc_T)b[0]);
  acc_T ey1 = min((acc_T)a[1], (acc_T)b[1]);
  acc_T ex2 = max((acc_T)a[2], (acc_T)b[2]);
  acc_T ey2 = max((acc_T)a[3], (acc_T)b[3]);

  acc_T cw = ex2 - ex1;
  acc_T ch = ey2 - ey1;
  acc_T c2 = cw*cw + ch*ch + (acc_T)1e-7;

  acc_T diou = iou - rho2 / c2;

  return (diou > (acc_T)threshold);
}
...
__global__ void nms_kernel_impl(
  ...
  if (threadIdx.x < row_size) {
    ...
    for (i = start; i < col_size; i++) {
      if (devDIoU<T>(cur_box, block_boxes + i * 4, iou_threshold)) {
        t |= 1ULL << i;
      }
    }
