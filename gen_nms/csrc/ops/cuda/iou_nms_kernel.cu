...
template <typename T>
__global__ void iou_nms_kernel_impl(
    int n_boxes,
...
at::Tensor iou_nms_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      dets_sorted.scalar_type(), "iou_nms_kernel", [&] {
        iou_nms_kernel_impl<scalar_t><<<blocks, threads, 0, stream>>>(
            dets_num,
            iou_threshold,
...
TORCH_LIBRARY_IMPL(gen_nms, CUDA, m) {
  m.impl("iou_nms", TORCH_FN(iou_nms_kernel));
}
